...
def setup_gui():
    ...
    stat_fields = {"level"} | {"strength", "dexterity", "constitution", "intelligence", "wisdom", "charisma"}
    ...
    for i, field in enumerate(fields):
        ...
        if field in stat_fields:
            ...
            def make_random_setter(f=field):
                def set_random():
                    entry_field = entries[f]
                    entry_field.delete(0, tk.END)
                    if f == "level":
                        entry_field.insert(0, str(random.randint(1, 20)))
                    else:
                        entry_field.insert(0, str(random.randint(3, 18)))

                return set_random

            rand_button = tk.Button(frame, text="🎲", command=make_random_setter())
            rand_button.pack(side="left", padx=5)
